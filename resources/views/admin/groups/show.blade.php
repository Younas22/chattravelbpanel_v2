...
<div class="flex flex-1 gap-4 min-w-0" x-data="groupChat({{ $group->id }})">
    ...
    <div class="w-72 shrink-0 space-y-4 overflow-y-auto">
        ...
        <div class="card">
            ...
            @else
                <form method="POST" action="{{ route('admin.groups.members.add', $group) }}" class="flex gap-2">
                    @csrf
                    <select name="ticket_user_id" class="flex-1 px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @foreach($availableUsers as $user)
                            <option value="{{ $user->id }}">{{ $user->full_name }} ({{ $user->email }})</option>
                        @endforeach
                    </select>
                    <button type="submit" class="btn-primary !py-2 !px-3 cursor-pointer">Add</button>
                </form>
            @endif
        </div>
    </div>
