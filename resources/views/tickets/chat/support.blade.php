...
<div class="flex h-full gap-4" x-data="supportChat()">
    ...
    <div class="flex flex-col flex-1 card !p-0 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-3" id="messages-container">
            ...
            <div class="group flex items-start gap-2 {{ $msg->sender_type === 'ticket_user' ? 'justify-start' : 'justify-end' }}">
                @if($msg->sender_type !== 'ticket_user')
                <button onclick="supportSetReply({{ $msg->id }}, {{ json_encode($msg->body ?: ($msg->attachment_name ?? '')) }}, '{{ $msg->sender_type }}')"
                ...
                <div class="max-w-[70%]">
                    ...
                    @if($msg->replyTo)
                    <div class="mb-1 px-3 py-1.5 rounded-xl border-l-4 {{ $msg->sender_type === 'ticket_user' ? 'border-white/50 bg-blue-500/30' : 'border-blue-400 bg-slate-100' }} text-xs text-slate-500 truncate">
                        <span class="font-semibold {{ $msg->sender_type === 'ticket_user' ? 'text-white/80' : 'text-blue-600' }}">
                            {{ $msg->replyTo->sender_type === 'admin' ? 'Support Team' : 'You' }}
                        </span>
                        <span class="{{ $msg->sender_type === 'ticket_user' ? 'text-white/70' : 'text-slate-500' }}">
                    ...
                    @if($msg->body)
                        <div class="px-4 py-2.5 rounded-2xl text-sm break-words {{ $msg->sender_type === 'ticket_user' ? 'bg-blue-600 text-white rounded-bl-sm' : 'bg-slate-100 text-slate-900 rounded-br-sm' }}" style="overflow-wrap:break-word;word-break:break-word;">
                            {!! nl2br(e($msg->body)) !!}
                        </div>
                    @endif
                ...
                <div class="group flex items-start gap-2" :class="msg.is_mine ? 'justify-start' : 'justify-end'">
                    ...
                    <div class="max-w-[70%]">
                        ...
                        <div x-show="msg.reply_to" class="mb-1 px-3 py-1.5 rounded-xl border-l-4 text-xs truncate"
                            ...
                            <span class="font-semibold" x-text="msg.reply_to?.is_mine ? 'You' : 'Support Team'"></span>
        ...
        <div class="border-t border-slate-100 px-4 pt-3 pb-4">
            ...
            <div x-show="replyTo" x-cloak class="mb-2 flex items-center gap-2 pl-3 pr-2 py-2 bg-blue-50 border-l-4 border-blue-500 rounded-xl">
                <div class="flex-1 min-w-0">
                    <p class="text-[11px] font-semibold text-blue-600" x-text="replyTo?.sender_type === 'admin' ? 'Replying to Support Team' : 'Replying to yourself'"></p>
...
function supportSetReply(id, body, senderType) {
    if (window._supportChatInstance) window._supportChatInstance.setReply({ id, body, sender_type: senderType, is_mine: senderType === 'ticket_user' });
}
...
function supportChat() {
    return {
        ...
        setReply(msg) {
            this.replyTo = { id: msg.id, body: msg.body, sender_type: msg.is_mine ? 'ticket_user' : 'admin', attachment_name: msg.attachment_name };
        ...
        init() {
            window._supportChatInstance = this;
                ...
                .then(data => {
                    if (data.message) {
                        self.newMessages.push({
                            ...
                            reply_to: self.replyTo ? { id: self.replyTo.id, body: self.replyTo.body, is_mine: self.replyTo.sender_type === 'ticket_user', attachment_name: self.replyTo.attachment_name } : null,
                        });
