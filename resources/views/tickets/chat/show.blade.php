    ...
    <div class="flex flex-col flex-1 card !p-0 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-3" id="messages-container">
            ...
            <div class="group flex items-start gap-2 {{ $msg->sender_type === 'admin' ? 'justify-start' : 'justify-end' }}">
                ...
                @else
                <button onclick="groupSetReply({{ $msg->id }}, {{ json_encode($msg->body ?: ($msg->attachment_name ?? '')) }}, {{ $msg->sender_id }}, {{ json_encode($msg->sender_name) }}, '{{ $msg->sender_type }}')"
                ...
                <div class="max-w-[70%]">
                    ...
                    <div class="mb-1 px-3 py-1.5 rounded-xl border-l-4 {{ $msg->sender_type === 'admin' ? 'border-white/50 bg-blue-500/30' : 'border-blue-400 bg-slate-100' }} text-xs text-slate-500 truncate">
                        <span class="font-semibold {{ $msg->sender_type === 'admin' ? 'text-white/80' : 'text-blue-600' }}">
                            {{ $msg->replyTo->sender_type === 'admin' ? 'Support Team' : ($msg->replyTo->sender_id === auth('ticket_user')->id() ? 'You' : $msg->replyTo->sender_name) }}
                        </span>
                ...
                @if($msg->sender_type === 'admin')
                <button onclick="groupSetReply({{ $msg->id }}, {{ json_encode($msg->body ?: ($msg->attachment_name ?? '')) }}, {{ $msg->sender_id }}, 'Support Team', '{{ $msg->sender_type }}')"
                    class="opacity-0 group-hover:opacity-100 transition-opacity mt-5 p-1 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 cursor-pointer shrink-0">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>
                </button>
                @else
                ...
                <div class="group flex items-start gap-2" :class="msg.sender_type === 'admin' ? 'justify-start' : 'justify-end'">
                    ...
                    <div class="max-w-[70%]">
                        ...
                        <div x-show="msg.reply_to" class="mb-1 px-3 py-1.5 rounded-xl border-l-4 text-xs truncate"
                            ...
                            <span class="font-semibold" x-text="msg.reply_to?.sender_type === 'admin' ? 'Support Team' : (msg.reply_to?.sender_id === currentUserId ? 'You' : msg.reply_to?.sender_name)"></span>
        ...
        <div class="border-t border-slate-100 px-4 pt-3 pb-4">
            ...
            <div x-show="replyTo" x-cloak class="mb-2 flex items-center gap-2 pl-3 pr-2 py-2 bg-blue-50 border-l-4 border-blue-500 rounded-xl">
                <div class="flex-1 min-w-0">
                    <p class="text-[11px] font-semibold text-blue-600" x-text="'Replying to ' + (replyTo?.sender_type === 'admin' ? 'Support Team' : (replyTo?.sender_id === currentUserId ? 'You' : replyTo?.sender_name))"></p>
...
function groupSetReply(id, body, senderId, senderName, senderType) {
    if (window._groupChatInstance) window._groupChatInstance.setReply({ id, body, sender_id: senderId, sender_name: senderName, sender_type: senderType });
}
...
function groupChat(groupId) {
    return {
        ...
        showEmoji: false,
        replyTo: null,
        currentUserId: {{ auth('ticket_user')->id() }},
        pollUrl: '{{ route('tickets.chat.poll', $group) }}',
        sendUrl: '{{ route('tickets.chat.message', $group) }}',

        setReply(msg) {
            this.replyTo = { id: msg.id, body: msg.body, sender_type: msg.sender_type, sender_id: msg.sender_id, sender_name: msg.sender_name, attachment_name: msg.attachment_name };
                ...
                .then(data => {
                    if (data.message) {
                        self.newMessages.push({
                            ...
                            reply_to: self.replyTo ? { id: self.replyTo.id, body: self.replyTo.body, sender_type: self.replyTo.sender_type, sender_id: self.replyTo.sender_id, sender_name: self.replyTo.sender_name, attachment_name: self.replyTo.attachment_name } : null,
                        });
