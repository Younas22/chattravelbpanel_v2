  ...
  function injectStyles() {
    ...
    const css = `
      ...
      * { box-sizing: border-box; font-family: 'Inter', sans-serif; margin: 0; padding: 0; }
      ...
      #tbp-avatar-wrap { position: relative; flex-shrink: 0; }
      #tbp-avatar { width: 38px; height: 38px; border-radius: 50%; background: rgba(255,255,255,0.2); display: flex; align-items: center; justify-content: center; font-weight: 700; color: ${t}; font-size: 15px; overflow: hidden; }
      #tbp-avatar-status { position: absolute; bottom: -1px; right: -1px; width: 11px; height: 11px; background: #4ade80; border-radius: 50%; border: 2px solid ${p}; }
        ...
        <div id="tbp-header">
          <div id="tbp-header-info">
            <div id="tbp-avatar-wrap">
              ...
              ${settings.show_online_status === 'true' ? '<span id="tbp-avatar-status"></span>' : ''}
            </div>
            <div>
...
(function () {
  ...
  var BASE = _s ? new URL(_s.src).origin + new URL(_s.src).pathname.replace(/\/widget\.js(\?.*)?$/, '') : '';
  ...
  function buildPopup(data) {
    ...
    if (!document.getElementById('tbp-offer-style')) {
      ...
      st.textContent = [
        ...
        '#tbp-offer-card{width:280px;background:#fff;border-radius:0 20px 20px 0;box-shadow:4px 8px 40px rgba(0,0,0,.18);overflow:hidden}',
        '.tbp-oh{background:linear-gradient(135deg,#f97316,#ef4444);padding:14px 14px 10px;position:relative}',
        '.tbp-oh-close{position:absolute;top:8px;right:8px;background:rgba(255,255,255,.25);border:none;border-radius:50%;width:22px;height:22px;cursor:pointer;color:#fff;font-size:13px;display:flex;align-items:center;justify-content:center;transition:.15s}',
        '.tbp-oh-close:hover{background:rgba(255,255,255,.4)}',
        ...
        '.tbp-oh-title{color:#fff;font-size:15px;font-weight:700;line-height:1.3;margin-bottom:2px;padding-right:24px}',
        '.tbp-oh-sub{color:rgba(255,255,255,.75);font-size:11px}',
        '.tbp-ob{padding:12px 14px}',
        '.tbp-prices{display:flex;align-items:baseline;gap:8px;margin-bottom:6px}',
        '.tbp-price-new{font-size:26px;font-weight:800;color:#0f172a;line-height:1}',
        '.tbp-price-old{font-size:14px;color:#94a3b8;text-decoration:line-through}',
        '.tbp-save-badge{display:inline-block;background:#fef2f2;color:#ef4444;font-size:11px;font-weight:700;padding:2px 8px;border-radius:20px;margin-bottom:10px}',
        '.tbp-cdown-wrap{background:#f8fafc;border-radius:12px;padding:8px 10px;margin-bottom:10px;text-align:center}',
        '.tbp-cdown-label{font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px}',
        '.tbp-cdown{font-size:22px;font-weight:800;color:#0f172a;letter-spacing:.05em;font-variant-numeric:tabular-nums}',
        ...
        '.tbp-btn-demo:hover{border-color:#f97316;color:#f97316}',
        ...
        '.tbp-btn-contact:hover{opacity:.88}'
      ].join('');
    ...
    wrap.innerHTML =
      '<div id="tbp-offer-card">' +
        '<div class="tbp-oh">' +
          '<button class="tbp-oh-close" id="tbp-offer-close">&#10005;</button>' +
          tagHtml +
        ...
        '<div class="tbp-ob">' +
          ...
          '<div class="tbp-actions">' +
            '<a href="' + DEMO_URL + '" target="_blank" rel="noopener" class="tbp-btn-demo" id="tbp-offer-demo">Demo</a>' +
            '<a href="' + CONTACT_URL + '" target="_blank" rel="noopener" class="tbp-btn-contact" id="tbp-offer-contact">Contact Us</a>' +
          '</div>' +
        '</div>' +
      '</div>';
